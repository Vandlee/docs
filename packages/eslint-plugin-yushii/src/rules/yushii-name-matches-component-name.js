...
const rule = {
  meta: {
    messages: {
      nameMismatch:
        "Expected `name` to be 'Yushii{{ componentName }}' but instead got '{{ name }}'.",
      ...
      noNameSecondArgument:
        "Unable to find name argument. Expected `{{ customHook }}(firstParameter, 'YushiComponent')`.",
      noNameValue:
    ...
    return {
      CallExpression(node) {
        ...
        if (nameLiteral !== null) {
          ...
          } else if (name !== `Yushi${componentName}`) {
            context.report({
              node: nameLiteral,
