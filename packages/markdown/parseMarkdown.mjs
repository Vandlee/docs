...
function getContents(markdown) {
  const rep = markdown
    ...
    .split(/^{{("(?:demo|component|codesandbox)":.*)}}$/gm) // Split markdown into an array, separating demos
...
function createRender(context) {
  const { headingHashes = {}, toc = [], userLanguage = 'es', options } = context;
  ...
  function render(markdown) {
    ...
    renderer.link = function link({ href, title, tokens }) {
      ...
      if (userLanguage !== 'es' && href.startsWith('/') && !options.ignoreLanguagePages(href)) {
