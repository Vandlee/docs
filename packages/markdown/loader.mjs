...
const notSpanishMarkdownRegExp = /-([a-z]{2})\.md$/;
...
export default async function demoLoader() {
  const spanishFilepath = this.resourcePath;
  ...
  const spanishFilename = path.basename(spanishFilepath, '.md');

  const files = await fs.readdir(path.dirname(spanishFilepath));
  const translations = await Promise.all(
    files
      .map((filename) => {
        if (filename === `${spanishFilename}.md`) {
          return {
            filename,
            userLanguage: 'es',
          };
        ...
        const matchNotSpanishMarkdown = filename.match(notSpanishMarkdownRegExp);

        if (
          filename.startsWith(spanishFilename) &&
          matchNotSpanishMarkdown !== null &&
          options.languagesInProgress.includes(matchNotSpanishMarkdown[1])
        ) {
          return {
            filename,
            userLanguage: matchNotSpanishMarkdown[1],
          };
        }
      ...
      .map(async (translation) => {
        const filepath = path.join(path.dirname(spanishFilepath), translation.filename);
  ...
  componentNames.forEach((componentName) => {
    const moduleID = componentName.startsWith('@vandlee/docs/')
      ? componentName
