...
import { getLineFeed } from '@yushii/internal-docs-utils';
...
export function getSystemComponents() {
  if (!systemComponents) {
    systemComponents = fs
      .readdirSync(path.resolve('packages', 'yushii-system', 'src'))
...
export function getYushiiName(name: string) {
  return `Yushii${name.replace('Styled', '')}`;
}
...
export function extractPackageFile(filePath: string) {
  ...
  return {
    ...result,
    yushiiPackage: result.packagePath?.replace('x-', 'yushii-'),
  };
}
