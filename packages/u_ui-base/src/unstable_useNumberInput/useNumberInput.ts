...
import { unstable_useForkRef as useForkRef, unstable_useId as useId } from '@vandlee/utils';
import { extractEventHandlers } from '../utils/extractEventHandlers';
import { VandleeCancellableEvent } from '../utils/vandleeCancellableEvent';
...
export function useNumberInput(parameters: UseNumberInputParameters): UseNumberInputReturnValue {
  ...
  const handleInputRefWarning = React.useCallback((instance: HTMLElement) => {
    if (process.env.NODE_ENV !== 'production') {
      if (instance && instance.nodeName !== 'INPUT' && !instance.focus) {
        console.error(
          [
            'VANDLEE: You have provided a `slots.input` to the input component',
  ...
  const createHandleFocus =
    ...
    (event: React.FocusEvent<HTMLInputElement> & VandleeCancellableEvent) => {
      otherHandlers.onFocus?.(event);

      if (event.defaultVandleePrevented || event.defaultPrevented) {
        return;
      }
  ...
  const createHandleInputChange =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.ChangeEvent<HTMLInputElement> & VandleeCancellableEvent) => {
  ...
  const createHandleClick =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.MouseEvent<HTMLInputElement> & VandleeCancellableEvent) => {
  ...
  const createHandleKeyDown =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.KeyboardEvent<HTMLInputElement> & VandleeCancellableEvent) => {
