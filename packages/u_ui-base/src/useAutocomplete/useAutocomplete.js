...
export function useAutocomplete(props) {
  ...
  getOptionLabel = (option) => {
    ...
    if (typeof optionLabel !== 'string') {
      if (process.env.NODE_ENV !== 'production') {
        ...
        console.error(
          `VANDLEE: The \`getOptionLabel\` method of ${componentName} returned ${erroneousReturn} instead of a string for ${JSON.stringify(
            option,
          )}.`,
        );
      }
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (value !== null && !freeSolo && options.length > 0) {
      ...
      if (missingValue.length > 0) {
        console.warn(
          [
            `VANDLEE: The value provided to ${componentName} is invalid.`,
    ...
    React.useEffect(() => {
      if (!inputRef.current || inputRef.current.nodeName !== 'INPUT') {
        if (inputRef.current && inputRef.current.nodeName === 'TEXTAREA') {
          ...
          console.error(
            [
              `VANDLEE: Unable to find the input element. It was resolved to ${inputRef.current} while an HTMLInputElement was expected.`,
    ...
    if (multiple) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (matches.length > 1) {
          console.error(
            [
              `VANDLEE: The \`isOptionEqualToValue\` method of ${componentName} does not handle the arguments correctly.`,
  ...
  const handleKeyDown = (other) => (event) => {
    ...
    if (event.defaultVandleePrevented) {
      return;
    }
      ...
      if (acc.length > 0 && acc[acc.length - 1].group === group) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          if (indexBy.get(group) && !warn) {
            console.warn(
              `VANDLEE: The options provided combined with the \`groupBy\` method of ${componentName} returns duplicated headers.`,
