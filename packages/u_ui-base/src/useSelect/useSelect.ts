...
function useSelect<OptionValue, Multiple extends boolean = false>(
  ...
  const createHandleButtonClick =
    (externalEventHandlers?: EventHandlers) => (event: React.MouseEvent & VandleeCancellableEvent) => {
  ...
  const createHandleButtonKeyDown =
    (otherHandlers: EventHandlers) => (event: React.KeyboardEvent & VandleeCancellableEvent) => {
  ...
  const createListboxHandleBlur =
    (otherHandlers: EventHandlers) => (event: React.FocusEvent & VandleeCancellableEvent) => {
      ...
      if (
        ...
        event.defaultVandleePrevented = true;
      }
    };
