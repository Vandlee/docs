...
export function unstable_createUseMediaQuery(params: { themeId?: string } = {}) {
  ...
  return function useMediaQuery<Theme = unknown>(
    ...
    // All the browsers Yushii support have this built-in.
    ...
    const {
      ...
      ssrMatchMedia = null,
      noSsr = false,
    } = getThemeProps({ name: 'YushiiUseMediaQuery', props: options, theme });

    if (process.env.NODE_ENV !== 'production') {
      if (typeof queryInput === 'function' && theme === null) {
        console.error(
          [
            'Yushii: The `query` argument provided is invalid.',
