...
import { GlobalStyles } from '@yushii/styled-engine';
import { useTheme as yushiiUseTheme } from '@mui/private-theming';
...
export default function createCssVarsProvider(options) {
  ...
  function CssVarsProvider(props) {
    ...
    const upperTheme = yushiiUseTheme();
    ...
    React.useEffect(() => {
      if (
        ...
        if (selector?.startsWith('data-') && !selector.includes('%s')) {
          // 'data-yushii-color-scheme' -> '[data-yushii-color-scheme="%s"]'
          rule = `[${selector}="%s"]`;
        }
        if (rule.startsWith('.')) {
          ...
          process.env.NODE_ENV === 'production'
            ...
            : (newMode) => {
                if (memoTheme.colorSchemeSelector === 'media') {
                  console.error(
                    [
                      'Yushii: The `setMode` function has no effect if `colorSchemeSelector` is `media` (`media` is the default value).',
