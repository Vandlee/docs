...
import HTMLElementType from '@yushii/utils/HTMLElementType';
import refType from '@yushii/utils/refType';
import elementTypeAcceptingRef from '@yushii/utils/elementTypeAcceptingRef';
import integerPropType from '@yushii/utils/integerPropType';
...
const Popover = React.forwardRef(function Popover(inProps, ref) {
  const props = useDefaultProps({ props: inProps, name: 'YushiiPopover' });
  const {
    action,
    anchorEl,
    anchorOrigin = {
  ...
  const getAnchorOffset = React.useCallback(() => {
    if (anchorReference === 'anchorPosition') {
      if (process.env.NODE_ENV !== 'production') {
        if (!anchorPosition) {
          console.error(
            'Yushii: You need to provide a `anchorPosition` prop when using ' +
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (
        ...
        console.warn(
          [
            'Yushii: The `anchorEl` prop provided to the component is invalid.',
    ...
    (element) => {
      ...
      if (process.env.NODE_ENV !== 'production') {
        if (elemRect.height > heightThreshold && elemRect.height && heightThreshold) {
          console.error(
            [
              'Yushii: The popover component is too tall.',
  ...
  if (transitionDurationProp === 'auto' && !TransitionComponent.yushiiSupportAuto) {
