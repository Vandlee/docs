...
import { unstable_createGetCssVar as systemCreateGetCssVar, createSpacing } from '@yushii/system';
import { createUnarySpacing } from '@yushii/system/spacing';
import {
  prepareCssVars,
  prepareTypographyVars,
  createGetColorSchemeSelector,
} from '@yushii/system/cssVars';
...
import {
  ...
  hslToRgb,
} from '@yushii/system/colorManipulator';
...
function setColorChannel(obj, key) {
  if (!(`${key}Channel` in obj)) {
    ...
    obj[`${key}Channel`] = safeColorChannel(
      toRgb(obj[key]),
      `Yushii: Can't create \`palette.${key}Channel\` because \`palette.${key}\` is not one of these formats: #nnn, #nnnnnn, rgb(), rgba(), hsl(), hsla(), color().` +
        '\n' +
...
export const createGetCssVar = (cssVarPrefix = 'yushii') => systemCreateGetCssVar(cssVarPrefix);
...
function attachColorScheme(colorSchemes, scheme, restTheme, colorScheme) {
  ...
  const { palette, ...yushiiTheme } = createThemeNoVars({
    ...restTheme,
    palette: { mode, ...scheme?.palette },
  });
...
export default function createThemeWithVars(options = {}, ...args) {
  ...
  if (!defaultScheme) {
    throw /* minify-error */ new Error(
      `Yushii: The \`colorSchemes.${defaultColorScheme}\` option is either missing or invalid.`,
    );
  }

  // Create the palette for the default color scheme, either `light`, `dark`, or custom color scheme.
  const yushiiTheme = attachColorScheme(colorSchemes, defaultScheme, input, defaultColorScheme);
  ...
  let theme = {
    ...
    getCssVar,
    colorSchemes,
    font: { ...prepareTypographyVars(yushiiTheme.typography), ...yushiiTheme.font },
