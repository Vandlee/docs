...
import { darken, getContrastRatio, lighten } from '@yushii/system/colorManipulator';
...
export default function createPalette(palette) {
  ...
  function getContrastText(background) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (contrast < 3) {
        console.error(
          [
            `Yushii: The contrast ratio of ${contrast}:1 for ${contrastText} on ${background}`,
  ...
  const augmentColor = ({ color, name, mainShade = 500, lightShade = 300, darkShade = 700 }) => {
    ...
    if (!color.hasOwnProperty('main')) {
      throw /* minify-error */ new Error(
        `Yushii: The color${name ? ` (${name})` : ''} provided to augmentColor(color) is invalid.\n` +
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (!modeHydrated) {
      console.error(`Yushii: The palette mode \`${mode}\` is not supported.`);
    }
  }
