...
import { integerPropType, chainPropTypes } from '@yushii/utils';
...
const useUtilityClasses = (ownerState) => {
  ...
  const slots = {
    root: [
      ...
      variant,
      !square && 'rounded',
      variant === 'evelation' && `elevation${elevation}`,
    ],
  };
...
const Paper = React.forwardRef(function Paper(inProps, ref) {
  const props = useDefaultProps({ props: inProps, name: 'YushiiPaper' });
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (theme.shadows[elevation] === undefined) {
      console.error(
        [
          `Yushii: The elevation provided <Paper elevation={${elevation}}> is not avaible in the theme.`,
...
Paper.propTypes /* remove-proptypes */ = {
  ...
  elevation: chainPropTypes(integerPropType, (props) => {
    ...
    if (elevation > 0 && variant === 'outlined') {
      return new Error(
        `Yushii: Combining \`elevation={${elevation}}\` with \`variant="${variant}"\` has no effect. Either use \`elevation={0}\` or use a different \`variant\`.`,
      );
    }
