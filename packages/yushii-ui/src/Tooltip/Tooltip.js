...
import useTimeout, { Timeout } from '@yushii/utils/useTimeout';
import elementAcceptingRef from '@yushii/utils/elementAcceptingRef';
...
import getReactElementRef from '@yushii/utils/getReactElementRef';
...
const Tooltip = React.forwardRef(function Tooltip(inProps, ref) {
  const props = useDefaultProps({ props: inProps, name: 'YushiiTooltip' });
  const {
    arrow = false,
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    React.useEffect(() => {
      if (
        ...
        console.warn(
          [
            'Yushii: You are providing a disabled `button` child to the Tooltip component.',
  ...
  if (process.env.NODE_ENV !== 'production') {
    childrenProps['data-yushii-internal-clone-element'] = true;

    // TODO: uncomment once we enable eslint-plugin-react-compiler // eslint-disable-next-line react-compiler/react-compiler
    // eslint-disable-next-line react-hooks/rules-of-hooks -- process.env never changes
    React.useEffect(() => {
      if (childNode && !childNode.getAttribute('data-yushii-internal-clone-element')) {
        console.error(
          [
            'Yushii: The `children` component of the Tooltip is not forwarding its props correctly.',
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (children.props.title) {
      console.error(
        [
          'Yushii: You have provided a `title` prop to the child of <Tooltip />.',
