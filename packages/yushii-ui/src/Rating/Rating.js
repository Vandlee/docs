...
import clamp from '@yushii/utils/clamp';
import visuallyHidden from '@yushii/utils/visuallyHidden';
...
const RatingRoot = styled('span', {
  ...
  memoTheme(({ theme }) => ({
    ...
    variants: [
      ...
      {
        // TODO v6: use the .Yushii-readOnly global state class
...
const Rating = React.forwardRef(function Rating(inProps, ref) {
  const props = useDefaultProps({ name: 'YushiiRating', props: inProps });
  const {
    component = 'span',
    className,
    defaultValue = null,
  ...
  return (
    <RatingRoot
      ...
      className={clsx(
        classes.root,
        {
          // TODO v6: remove this class as it duplicates with the global state class Yushii-readOnly
          'YushiiRating-readOnly': readOnly,
        },
        className,
      )}
...
Rating.propTypes /* remove-proptypes */ = {
  ...
  precision: chainPropTypes(PropTypes.number, (props) => {
    if (props.precision < 0.1) {
      return new Error(
        [
          'Yushii: The prop `precision` should be above 0.1.',
