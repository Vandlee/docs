  ...
  return {
    ...
    onMouseLeave: (event) => {
      if (rootNode.current === event.currentTarget) {
        (rootNode.current.querySelector('.YushiiCode-copy') as null | HTMLButtonElement)?.blur();
  ...
  React.useEffect(() => {
    ...
    if (codeRoots !== null) {
      ...
      Array.from(codeRoots).forEach((elm) => {
        ...
        const btn = elm.querySelector('.YushiiCode-copy') as HTMLButtonElement | null;
        if (btn) {
          const keyNode = btn.querySelector('.YushiiCode-copyKeypress')?.childNodes[1];
          if (!keyNode) {
...
export function CodeCopyProvider({ children }: CodeCopyProviderProps) {
  ...
  React.useEffect(() => {
    document.addEventListener('keydown', (event) => {
      ...
      const copyBtn = rootNode.current.querySelector('.YushiiCode-copy') as HTMLButtonElement;
