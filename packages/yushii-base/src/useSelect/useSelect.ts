...
function useSelect<OptionValue, Multiple extends boolean = false>(
  ...
  const createHandleButtonClick =
    (externalEventHandlers?: EventHandlers) => (event: React.MouseEvent & YushiiCancellableEvent) => {
  ...
  const createHandleButtonKeyDown =
    (otherHandlers: EventHandlers) => (event: React.KeyboardEvent & YushiiCancellableEvent) => {
  ...
  const createListboxHandleBlur =
    (otherHandlers: EventHandlers) => (event: React.FocusEvent & YushiiCancellableEvent) => {
      ...
      if (
        ...
        event.defaultYushiiPrevented = true;
      }
    };
