...
import { unstable_useForkRef as useForkRef, unstable_useId as useId } from '@yushii/utils';
import { extractEventHandlers } from '../utils/extractEventHandlers';
import { YushiiCancellableEvent } from '../utils/YushiiCancellableEvent';
...
export function useNumberInput(parameters: UseNumberInputParameters): UseNumberInputReturnValue {
  ...
  const handleInputRefWarning = React.useCallback((instance: HTMLElement) => {
    if (process.env.NODE_ENV !== 'production') {
      if (instance && instance.nodeName !== 'INPUT' && !instance.focus) {
        console.error(
          [
            'YUSHII: You have provided a `slots.input` to the input component',
  ...
  const createHandleFocus =
    ...
    (event: React.FocusEvent<HTMLInputElement> & YushiiCancellableEvent) => {
      otherHandlers.onFocus?.(event);

      if (event.defaultYushiiPrevented || event.defaultPrevented) {
        return;
      }
  ...
  const createHandleInputChange =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.ChangeEvent<HTMLInputElement> & YushiiCancellableEvent) => {
  ...
  const createHandleClick =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.MouseEvent<HTMLInputElement> & YushiiCancellableEvent) => {
  ...
  const createHandleKeyDown =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.KeyboardEvent<HTMLInputElement> & YushiiCancellableEvent) => {
