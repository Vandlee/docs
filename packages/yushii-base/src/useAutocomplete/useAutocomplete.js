...
export function useAutocomplete(props) {
  const {
    ...
    // eslint-disable-next-line @typescript-eslint/naming-convention
    unstable_classNamePrefix = 'Yushii',
    autoComplete = false,
    autoHighlight = false,
    autoSelect = false,
    blurOnSelect = false,
  ...
  getOptionLabel = (option) => {
    ...
    if (typeof optionLabel !== 'string') {
      if (process.env.NODE_ENV !== 'production') {
        ...
        console.error(
          `YUSHII: The \`getOptionLabel\` method of ${componentName} returned ${erroneousReturn} instead of a string for ${JSON.stringify(
            option,
          )}.`,
        );
      }
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (value !== null && !freeSolo && options.length > 0) {
      ...
      if (missingValue.length > 0) {
        console.warn(
          [
            `YUSHII: The value provided to ${componentName} is invalid.`,
    ...
    React.useEffect(() => {
      if (!inputRef.current || inputRef.current.nodeName !== 'INPUT') {
        if (inputRef.current && inputRef.current.nodeName === 'TEXTAREA') {
          ...
          console.error(
            [
              `YUSHII: Unable to find the input element. It was resolved to ${inputRef.current} while an HTMLInputElement was expected.`,
    ...
    if (multiple) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (matches.length > 1) {
          console.error(
            [
              `YUSHII: The \`isOptionEqualToValue\` method of ${componentName} does not handle the arguments correctly.`,
  ...
  const handleKeyDown = (other) => (event) => {
    ...
    if (event.defaultYushiiPrevented) {
      return;
    }
      ...
      if (acc.length > 0 && acc[acc.length - 1].group === group) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          if (indexBy.get(group) && !warn) {
            console.warn(
              `YUSHII: The options provided combined with the \`groupBy\` method of ${componentName} returns duplicated headers.`,
