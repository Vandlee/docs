...
import { GlobalStyles } from '@u-shii/styled-engine';
import { useTheme as ushiiUseTheme } from '@mui/private-theming';
...
export default function createCssVarsProvider(options) {
  ...
  function CssVarsProvider(props) {
    ...
    const upperTheme = ushiiUseTheme();
    ...
    React.useEffect(() => {
      if (
        ...
        if (selector?.startsWith('data-') && !selector.includes('%s')) {
          // 'data-u-shii-color-scheme' -> '[data-u-shii-color-scheme="%s"]'
          rule = `[${selector}="%s"]`;
        }
        if (rule.startsWith('.')) {
          ...
          process.env.NODE_ENV === 'production'
            ...
            : (newMode) => {
                if (memoTheme.colorSchemeSelector === 'media') {
                  console.error(
                    [
                      'U-SHII: The `setMode` function has no effect if `colorSchemeSelector` is `media` (`media` is the default value).',
