...
function useSelect<OptionValue, Multiple extends boolean = false>(
  ...
  const createHandleButtonClick =
    (externalEventHandlers?: EventHandlers) => (event: React.MouseEvent & UshiiCancellableEvent) => {
  ...
  const createHandleButtonKeyDown =
    (otherHandlers: EventHandlers) => (event: React.KeyboardEvent & UshiiCancellableEvent) => {
  ...
  const createListboxHandleBlur =
    (otherHandlers: EventHandlers) => (event: React.FocusEvent & UshiiCancellableEvent) => {
      ...
      if (
        ...
        event.defaultUshiiPrevented = true;
      }
    };
