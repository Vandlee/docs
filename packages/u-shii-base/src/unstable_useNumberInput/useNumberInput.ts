...
import { unstable_useForkRef as useForkRef, unstable_useId as useId } from '@u-shii/utils';
import { extractEventHandlers } from '../utils/extractEventHandlers';
import { UshiiCancellableEvent } from '../utils/u-shiiCancellableEvent';
...
export function useNumberInput(parameters: UseNumberInputParameters): UseNumberInputReturnValue {
  ...
  const handleInputRefWarning = React.useCallback((instance: HTMLElement) => {
    if (process.env.NODE_ENV !== 'production') {
      if (instance && instance.nodeName !== 'INPUT' && !instance.focus) {
        console.error(
          [
            'U-SHII: You have provided a `slots.input` to the input component',
  ...
  const createHandleFocus =
    ...
    (event: React.FocusEvent<HTMLInputElement> & UshiiCancellableEvent) => {
      otherHandlers.onFocus?.(event);

      if (event.defaultUshiiPrevented || event.defaultPrevented) {
        return;
      }
  ...
  const createHandleInputChange =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.ChangeEvent<HTMLInputElement> & UshiiCancellableEvent) => {
  ...
  const createHandleClick =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.MouseEvent<HTMLInputElement> & UshiiCancellableEvent) => {
  ...
  const createHandleKeyDown =
    (otherHandlers: Partial<EventHandlers>) =>
    (event: React.KeyboardEvent<HTMLInputElement> & UshiiCancellableEvent) => {
