...
import { GlobalStyles } from '@u_ui/styled-engine';
import { useTheme as vandleeUseTheme } from '@mui/private-theming';
...
export default function createCssVarsProvider(options) {
  ...
  function CssVarsProvider(props) {
    ...
    const upperTheme = vandleeUseTheme();
    ...
    React.useEffect(() => {
      if (
        ...
        if (selector?.startsWith('data-') && !selector.includes('%s')) {
          // 'data-u-ui-color-scheme' -> '[data-u-ui-color-scheme="%s"]'
          rule = `[${selector}="%s"]`;
        }
        if (rule.startsWith('.')) {
          ...
          process.env.NODE_ENV === 'production'
            ...
            : (newMode) => {
                if (memoTheme.colorSchemeSelector === 'media') {
                  console.error(
                    [
                      'VANDLEE: The `setMode` function has no effect if `colorSchemeSelector` is `media` (`media` is the default value).',
