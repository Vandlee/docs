...
function clampWrapper(value, min = 0, max = 1) {
  if (process.env.NODE_ENV !== 'production') {
    if (value < min || value > max) {
      console.error(`VANDLEE: The value provided ${value} is out of range [${min}, ${max}].`);
    }
  }
...
export function hexToRgb(color) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (color.length !== color.trim().length) {
      console.error(
        `VANDLEE: The color: "${color}" is invalid. Make sure the color input doesn't contain leading/trailing space.`,
      );
    }
  }
 ...
 * @returns {object} - A Vandlee color object: {type: string, values: number[]}
 */
export function decomposeColor(color) {
  ...
  if (!['rgb', 'rgba', 'hsl', 'hsla', 'color'].includes(type)) {
    throw /* minify-error */ new Error(
      `VANDLEE: Unsupported \`${color}\` color.\n` +
  ...
  if (type === 'color') {
    ...
    if (!['srgb', 'display-p3', 'a98-rgb', 'prophoto-rgb', 'rec-2020'].includes(colorSpace)) {
      throw /* minify-error */ new Error(
        `VANDLEE: unsupported \`${colorSpace}\` color space.\n` +
